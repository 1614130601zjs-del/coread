        ...
        <div className={`xiaowo-study ${readerTheme === 'eink' ? 'eink-theme' : `reader-theme-${readerTheme}`}`} style={{
            ...
            <div ref={contentRef} style={{
                ...
                onPointerDown={mode === 'reading' ? (e) => {
                    ...
                    curlEl.style.willChange = 'clip-path, transform, opacity, filter';
                    curlEl.style.opacity = '0';
                    curlEl.style.inset = '0';
                    curlEl.style.width = '100%';
                    curlEl.style.height = '100%';
                    curlEl.style.transform = 'translate3d(0,0,0)';
                    curlEl.style.transformOrigin = '50% 50%';
                    curlEl.style.clipPath = 'polygon(50% 50%, 50% 50%, 50% 50%)';
                ...
                onPointerMove={mode === 'reading' ? (e) => {
                    ...
                    if (pagePointerRafRef.current === null) {
                        pagePointerRafRef.current = window.requestAnimationFrame(() => {
                            ...
                            const localX = Math.max(0, Math.min(width, pagePointerXRef.current! - (rect?.left || 0)));
                            ...
                            if (current.grabMode === 'center') {
                                ...
                                const bodyPoly = current.direction === 'forward'
                                    ? [
                                        ...foldPoints.map(p => `${pct(p.x,width)} ${pct(p.y,height)}`),
                                        '100% 100%', '100% 0%'
                                    ].join(', ')
                                    : [
                                        '0% 0%', '0% 100%',
                                        ...foldPoints.slice().reverse().map(p => `${pct(p.x,width)} ${pct(p.y,height)}`)
                                    ].join(', ');

                                current.bodyEl.style.clipPath = `polygon(${bodyPoly})`;
                                current.curlEl.style.opacity = '1';
                                current.curlEl.style.inset = '0';
                                current.curlEl.style.width = '100%';
                                current.curlEl.style.height = '100%';
                                current.curlEl.style.clipPath = `polygon(${bodyPoly})`;
                ...
                onPointerUp={mode === 'reading' ? (e) => {
                    ...
                    if (!pageEl || !bodyEl || !curlEl || !canTurn) {
                        bodyEl && (bodyEl.style.clipPath = 'inset(0 0 0 0)');
                        if (curlEl) {
                            curlEl.style.opacity = '0';
                            curlEl.style.clipPath = 'polygon(50% 50%, 50% 50%, 50% 50%)';
                        }
                    ...
                    if (gesture.grabMode === 'center') {
                        ...
                        curlEl.style.transformOrigin =
                            direction === 'forward' ? '0% 50%' : '100% 50%';
                        curlEl.style.transform =
                            `perspective(1500px) translate3d(0,0,16px) rotateY(${direction === 'forward' ? -52 : 52}deg)`;
                        curlEl.style.filter =
                ...
                onPointerCancel={mode === 'reading' ? (e) => {
                    ...
                    if (curlEl) {
                        curlEl.style.transition = 'opacity 160ms ease-out, transform 160ms ease-out';
                        curlEl.style.opacity = '0';
                        curlEl.style.transform = '';
                        curlEl.style.clipPath = 'polygon(50% 50%, 50% 50%, 50% 50%)';
                    }
                            ...
                            >
                                <div
                                    ...
                                    style={{
                                        ...
                                        background: readerSurface,
                                        borderRadius: '0',
                                        boxShadow: '12px 0 30px rgba(60,45,30,.20), inset 0 0 22px rgba(255,255,255,.24)',
                                        transformStyle: 'preserve-3d',
                                        backfaceVisibility: 'hidden',
                                        isolation: 'isolate',
                                        contain: 'paint',
