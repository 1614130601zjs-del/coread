        ...
        <div className={`xiaowo-study ${readerTheme === 'eink' ? 'eink-theme' : `reader-theme-${readerTheme}`}`} style={{
            ...
            <div ref={contentRef} style={{
                ...
                onTouchStart={mode === 'reading' ? (e) => {
                    ...
                    const startedOnControl = Boolean(target?.closest('button, input, textarea, select, a, [role=\"button\"]'));
                    if (startedOnControl) {
                ...
                onTouchMove={mode === 'reading' ? (e) => {
                    ...
                    // First horizontal movement decides the page direction. There is no swipe threshold:
                    // once the finger moves, the paper follows it 1:1.
                    if (!start.direction && Math.abs(dx) > Math.abs(dy)) {
                        start.direction = dx < 0 ? 'forward' : 'backward';
                    ...
                    const pageEl = document.querySelector('[data-page-content]') as HTMLElement | null;
                    if (!pageEl) return;
                    const width = Math.max(pageEl.getBoundingClientRect().width, 1);
                    const signed = start.direction === 'forward' ? Math.min(0, dx) : Math.max(0, dx);
                    // A small rubber-band past the edge makes the gesture feel physical without allowing overscroll.
                    const maxDrag = width * 1.04;
                    const raw = Math.max(-maxDrag, Math.min(maxDrag, signed));
                    const atBoundary = start.direction === 'forward' ? page >= totalPages : page <= 1;
                    const clamped = atBoundary && Math.abs(raw) > width * 0.08 ? raw * 0.18 : raw;
                    pageEl.style.transform = `translate3d(${clamped}px, 0, 0)`;
                } : undefined}
                onTouchEnd={mode === 'reading' ? (e) => {
                    ...
                    // A tap is also a valid page-grab: right half -> next, left half -> previous.
                    if (!direction) {
                        direction = start.x >= window.innerWidth / 2 ? 'forward' : 'backward';
                    }
                    const canTurn = direction === 'forward' ? page < totalPages : page > 1;
                    const width = Math.max(pageEl.getBoundingClientRect().width, 1);
                    const targetX = canTurn ? (direction === 'forward' ? -width : width) : 0;
                    const settleMs = Math.max(160, Math.min(readerPageTurnDuration, 420));
                    pageEl.style.transition = `transform ${settleMs}ms cubic-bezier(0.2, 0.8, 0.2, 1)`;
                    ...
                    // Prevent a drag-release from becoming a second click.
                    if (Math.abs(dx) > 4 || Math.abs(dy) > 4) reserveSelectionGesture(700);
                } : undefined}
                    ...
                    <>
                        {readingLoading ? (
                            ...
                            <div
                                ...
                                style={{ '--coread-page-turn-duration': `${readerPageTurnDuration}ms` } as React.CSSProperties}
                            >
                                <div
                                    data-page-content
