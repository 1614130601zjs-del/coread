...
const StudyApp: React.FC = () => {
    ...
    // v4: Pointer Events 抓页状态。页面元素直接缓存，避免每一帧 querySelector。
            ...
            <div ref={contentRef} style={{
                ...
                onPointerDown={mode === 'reading' ? (e) => {
                    ...
                    const startedOnControl = Boolean(
                        target?.closest('button, input, textarea, select, a, [role="button"], [data-reader-control]')
                    );
                    if (startedOnControl) {
                    ...
                    const curlEl = pageEl?.querySelector('[data-page-curl-sheet]') as HTMLElement | null;
                    if (!pageEl || !bodyEl || !curlEl) return;
                    ...
                    curlEl.style.willChange = 'left, right, width, opacity, transform, clip-path';

                    bodyEl.style.clipPath = 'inset(0 0 0 0)';
                    curlEl.style.opacity = '0';
                    curlEl.style.left = 'auto';
                    curlEl.style.right = 'auto';
                    curlEl.style.width = '0px';
                    curlEl.style.transform = 'translate3d(0,0,0) rotateY(0deg)';
                ...
                onPointerMove={mode === 'reading' ? (e) => {
                    ...
                    if (pagePointerRafRef.current === null) {
                        pagePointerRafRef.current = window.requestAnimationFrame(() => {
                            pagePointerRafRef.current = null;

                            const current = pagePointerRef.current;
                            const x = pagePointerXRef.current;
                            if (!current || x === null || !current.bodyEl || !current.curlEl) return;

                            const width = Math.max(
                                current.pageEl?.getBoundingClientRect().width || window.innerWidth,
                                1
                            );

                            let rawDx = x - current.startX;
                            if (current.direction === 'forward') rawDx = Math.min(0, rawDx);
                            else rawDx = Math.max(0, rawDx);

                            const progress = Math.min(1, Math.abs(rawDx) / width);

                            // 当前页：只保留“还没被掀起来”的部分。
                            // 左滑（下一页）：右边逐渐被掀走；
                            // 右滑（上一页）：左边逐渐被掀走。
                            const edge = Math.max(0, Math.min(100, progress * 100));
                            current.bodyEl.style.clipPath =
                                current.direction === 'forward'
                                    ? `inset(0 ${edge}% 0 0)`
                                    : `inset(0 0 0 ${edge}%)`;

                            // 卷起的纸张是一张独立的“纸片”，它会随进度扩大并产生弧面。
                            const curlWidth = Math.max(54, Math.min(width * 0.72, width * (0.16 + progress * 0.72)));
                            const depth = Math.min(62, progress * 62);
                            const rotate = Math.min(78, progress * 78);
                            const lift = Math.min(26, progress * 26);

                            current.curlEl.style.opacity = String(Math.min(1, 0.72 + progress * 0.28));
                            current.curlEl.style.width = `${curlWidth}px`;

                            if (current.direction === 'forward') {
                                current.curlEl.style.left = `${width - curlWidth}px`;
                                current.curlEl.style.right = 'auto';
                                current.curlEl.style.transformOrigin = 'left center';
                                current.curlEl.style.transform =
                                    `perspective(1200px) translate3d(${Math.min(18, progress * 18)}px,0,${lift}px) rotateY(${-rotate}deg)`;
                                current.curlEl.style.clipPath =
                                    `ellipse(${Math.max(55, curlWidth * 0.92)}% 72% at 100% 50%)`;
                            } else {
                                current.curlEl.style.right = `${width - curlWidth}px`;
                                current.curlEl.style.left = 'auto';
                                current.curlEl.style.transformOrigin = 'right center';
                                current.curlEl.style.transform =
                                    `perspective(1200px) translate3d(${Math.max(-18, -progress * 18)}px,0,${lift}px) rotateY(${rotate}deg)`;
                                current.curlEl.style.clipPath =
                                    `ellipse(${Math.max(55, curlWidth * 0.92)}% 72% at 0% 50%)`;
                            }

                            current.curlEl.style.setProperty('--coread-curl-progress', String(progress));
                        });
                    }
                ...
                onPointerUp={mode === 'reading' ? (e) => {
                    ...
                    if (!pageEl || !bodyEl || !curlEl || !canTurn) {
                        if (bodyEl) bodyEl.style.clipPath = 'inset(0 0 0 0)';
                        if (curlEl) {
                            curlEl.style.opacity = '0';
                            curlEl.style.width = '0px';
                        }
                    ...
                    // 松手就翻：即使只点一下，也执行完整的翻页。
                    const settleMs = Math.max(
                        190,
                        Math.min(readerPageTurnDuration, 460)
                    );

                    bodyEl.style.transition =
                        `clip-path ${settleMs}ms cubic-bezier(0.18,0.82,0.18,1)`;
                    curlEl.style.transition =
                        `width ${settleMs}ms cubic-bezier(0.18,0.82,0.18,1), left ${settleMs}ms cubic-bezier(0.18,0.82,0.18,1), right ${settleMs}ms cubic-bezier(0.18,0.82,0.18,1), transform ${settleMs}ms cubic-bezier(0.18,0.82,0.18,1), clip-path ${settleMs}ms cubic-bezier(0.18,0.82,0.18,1)`;
                    ...
                    curlEl.style.width = `${width * 0.92}px`;
                    curlEl.style.opacity = '1';
                    curlEl.style.clipPath =
                        direction === 'forward'
                            ? 'ellipse(96% 72% at 100% 50%)'
                            : 'ellipse(96% 72% at 0% 50%)';
                    curlEl.style.transformOrigin =
                        direction === 'forward' ? 'left center' : 'right center';
                    curlEl.style.transform =
                        `perspective(1200px) translate3d(${direction === 'forward' ? 22 : -22}px,0,28px) rotateY(${direction === 'forward' ? -82 : 82}deg)`;
                ...
                onPointerCancel={mode === 'reading' ? (e) => {
                    ...
                    if (bodyEl) {
                        bodyEl.style.transition = 'clip-path 180ms ease-out';
                        bodyEl.style.clipPath = 'inset(0 0 0 0)';
                    }
                    if (curlEl) {
                        curlEl.style.transition = 'opacity 160ms ease-out, width 160ms ease-out';
                        curlEl.style.opacity = '0';
                        curlEl.style.width = '0px';
                    }
                } : undefined}>
                            ...
                            >
                                <div
                                    ...
                                    style={{
                                        ...
                                        background: readerSurface,
                                        borderRadius: '2px',
                                        boxShadow: '0 10px 30px rgba(60,45,30,.18), inset 0 0 18px rgba(255,255,255,.28)',
                                        transformStyle: 'preserve-3d',
                                        backfaceVisibility: 'hidden',
                                    } as React.CSSProperties}
                                >
                                    ...
                                    >
                                        {pageFragments.map((frag, visibleIndex) => {
                                            ...
                                            return (
                                                <div
                                                    key={`curl-${frag.idx}-${frag.startOffset}-${frag.endOffset}`}
                                                    style={{
                                                        marginBottom: chapterTitle ? CHAPTER_GAP_BOTTOM : readerLayout.paragraphGap,
                                                        marginTop: chapterTitle && visibleIndex > 0 ? CHAPTER_GAP_TOP : 0,
                                                    }}
                                    ...
                                    <div
                                        ...
                                        style={{
                                            ...
                                            inset: 0,
                                            pointerEvents: 'none',
                                            background: 'linear-gradient(90deg, rgba(255,255,255,.32), transparent 18%, rgba(0,0,0,.10) 68%, rgba(0,0,0,.24))',
