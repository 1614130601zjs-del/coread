...
const StudyApp: React.FC = () => {
    ...
    const pagePointerYRef = useRef<number | null>(null);
        ...
        <div className={`xiaowo-study ${readerTheme === 'eink' ? 'eink-theme' : `reader-theme-${readerTheme}`}`} style={{
            ...
            <div ref={contentRef} style={{
                ...
                onPointerDown={mode === 'reading' ? (e) => {
                    ...
                    pagePointerYRef.current = e.clientY;
                ...
                onPointerMove={mode === 'reading' ? (e) => {
                    ...
                    if (pagePointerRafRef.current === null) {
                        pagePointerRafRef.current = window.requestAnimationFrame(() => {
                            ...
                            const liveX = pagePointerXRef.current ?? current.startX;
                            const liveY = pagePointerYRef.current ?? current.startY;
                            const localX = Math.max(0, Math.min(width, liveX - (rect?.left || 0)));
                            const localY = Math.max(0, Math.min(height, liveY - (rect?.top || 0)));
                            const liveDx = liveX - current.startX;

                            const progress = Math.min(1, Math.max(0, Math.abs(liveDx) / width));
                ...
                onPointerCancel={mode === 'reading' ? (e) => {
                    ...
                    gesture.pageEl && (gesture.pageEl.style.clipPath = 'inset(0 0 0 0)');
                    if (gesture.curlEl) {
                        gesture.curlEl.style.opacity = '0';
                        gesture.curlEl.style.display = 'none';
                    }
                    pagePointerRef.current = null;
                    pagePointerXRef.current = null;
                    pagePointerYRef.current = null;
                } : undefined}
