        ...
        <div className={`xiaowo-study ${readerTheme === 'eink' ? 'eink-theme' : `reader-theme-${readerTheme}`}`} style={{
            ...
            <div ref={contentRef} style={{
                ...
                onPointerDown={mode === 'reading' ? (e) => {
                    ...
                    // 先让下一张/上一张纸尽可能进入缓存，拖动时底层不会突然空白。
                    const warmStartPages = [page - 1, page + 1].filter(
                        targetPage => targetPage >= 1 && targetPage <= totalPages
                    );
                    if (activeBook && paragraphCacheManifest && !paragraphsFullyLoaded) {
                        for (const targetPage of warmStartPages) {
                            const targetBreak = pageBreaks[targetPage - 1];
                            const sourceIdx = targetBreak?.paraIndex;
                            if (sourceIdx != null) {
                                void loadParagraphWindow(activeBook.id, paragraphCacheManifest, sourceIdx);
                            }
                        }
                    }
                    ...
                    pageEl.style.setProperty('--coread-curl-progress', '0');
                    pageEl.style.setProperty('--coread-curl-shadow', '0');
                    pageEl.style.setProperty('--coread-curl-direction', '1');
                ...
                onPointerMove={mode === 'reading' ? (e) => {
                    ...
                    // 合并高频 pointermove，避免手机上每个事件都强制触发样式更新。
                    if (pagePointerRafRef.current === null) {
                        pagePointerRafRef.current = window.requestAnimationFrame(() => {
                            ...
                            if (!current || x === null || !current.pageEl) return;

                            const width = Math.max(
                                current.pageEl.getBoundingClientRect().width,
                                1
                            );
                            let dxNow = x - current.startX;

                            const atBoundary =
                                (current.direction === 'forward' && page >= totalPages) ||
                                (current.direction === 'backward' && page <= 1);

                            if (current.direction === 'forward') {
                                dxNow = Math.min(0, dxNow);
                            } else {
                                dxNow = Math.max(0, dxNow);
                            }

                            if (atBoundary && Math.abs(dxNow) > width * 0.08) {
                                dxNow *= 0.18;
                            }

                            const progress = Math.min(1, Math.abs(dxNow) / width);
                            // 不是把整页“滑走”，而是模拟纸张被抓起后的弯折。
                            // 小位移时几乎保持平面；越接近完成翻页，Y 轴旋转越明显。
                            const bend = Math.min(72, progress * 72);
                            const signedBend = current.direction === 'forward' ? -bend : bend;
                            const followX = dxNow * (0.18 + progress * 0.12);
                            const liftZ = Math.min(34, progress * 34);
                            const skew = (current.direction === 'forward' ? 1 : -1) * progress * 1.8;

                            current.pageEl.style.transformOrigin =
                                current.direction === 'forward' ? 'left center' : 'right center';
                            current.pageEl.style.transform =
                                `perspective(1400px) translate3d(${followX}px, 0, ${liftZ}px) rotateY(${signedBend}deg) skewY(${skew}deg)`;
                            current.pageEl.style.setProperty('--coread-curl-progress', String(progress));
                            current.pageEl.style.setProperty('--coread-curl-shadow', String(Math.min(1, progress * 1.35)));
                            current.pageEl.style.setProperty('--coread-curl-direction', current.direction === 'forward' ? '1' : '-1');
                        });
                    }
                ...
                onPointerUp={mode === 'reading' ? (e) => {
                    ...
                    // 轻点也算抓页：右半屏下一页，左半屏上一页。
                    const direction =
                        gesture.direction ??
                        (e.clientX >= window.innerWidth / 2 ? 'forward' : 'backward');

                    const canTurn =
                        direction === 'forward' ? page < totalPages : page > 1;

                    const pageEl = gesture.pageEl;
                    const width = Math.max(pageEl?.getBoundingClientRect().width || 1, 1);
                    const threshold = Math.max(42, width * 0.20);
                    const shouldCommit =
                        canTurn &&
                        (Math.abs(dx) >= threshold || (!gesture.moved && Math.abs(dx) < 8));
                    ...
                    if (shouldCommit) {
                        ...
                        const settleMs = Math.max(
                            160,
                            Math.min(readerPageTurnDuration, 420)
                        );

                        pageEl.style.transition =
                            `transform ${settleMs}ms cubic-bezier(0.18, 0.82, 0.18, 1)`;
                        pageEl.style.transformOrigin =
                            direction === 'forward' ? 'left center' : 'right center';
                        pageEl.style.setProperty('--coread-curl-progress', '1');
                        pageEl.style.setProperty('--coread-curl-shadow', '1');
                        pageEl.style.transform =
                            `perspective(1400px) translate3d(${direction === 'forward' ? -width * 0.18 : width * 0.18}px, 0, 34px) rotateY(${direction === 'forward' ? -86 : 86}deg)`;

                        reserveSelectionGesture(1000);
                        ...
                        const settleMs = 180;
                        pageEl.style.transition =
                            `transform ${settleMs}ms cubic-bezier(0.22, 0.8, 0.24, 1)`;
                    ...
                    // 阻止拖动结束后紧接着触发普通 click。
                    if (Math.abs(dx) > 4 || Math.abs(dy) > 4) {
                        reserveSelectionGesture(700);
                    }
                ...
                onPointerCancel={mode === 'reading' ? (e) => {
                    ...
                    if (pageEl) {
                        pageEl.style.transition = 'transform 180ms ease-out';
                        pageEl.style.transform = 'translate3d(0, 0, 0)';
                        pageEl.style.setProperty('--coread-curl-progress', '0');
                        pageEl.style.setProperty('--coread-curl-shadow', '0');
                        window.setTimeout(() => {
