...
const loadGlobalReaderFontFace = async (font: GlobalReaderFont, sourceOverride?: string): Promise<boolean> => {
    if (typeof window === 'undefined' || !('FontFace' in window) || !document.fonts) return font.source === 'system';
...
const StudyApp: React.FC = () => {
    ...
    const [globalReaderFont, setGlobalReaderFont] = useState<GlobalReaderFont>(() => {
        try {
            ...
            if (!saved || !['system', 'upload', 'url'].includes(saved.source)) return { ...DEFAULT_GLOBAL_READER_FONT };
            return {
                        ...
                        {readerPanelFocus === 'typography' && (
                            <div style={{ marginBottom: 14, padding: 10, border: `1px solid ${readerBorder}`, borderRadius: 8, background: readerSurface }}>
                                ...
                                <div style={{ display: 'flex', gap: 6 }}>
                                    <input value={globalReaderFontUrl} onChange={e => setGlobalReaderFontUrl(e.target.value)} placeholder="或输入字体 URL" style={{ flex: 1, minWidth: 0, padding: '7px 8px', border: `1px solid ${readerBorder}`, borderRadius: 6, background: readerSurface, color: readerText, fontSize: 10, outline: 'none' }} />
                                ...
                                <div style={{ marginTop: 7, color: readerMuted, fontSize: 9, lineHeight: 1.45 }}>应用后作用于正文、章节标题和分页测量；刷新与换书后仍保留。</div>
                            </div>
                        )}
                        {readerPanelFocus === 'typography' && (
