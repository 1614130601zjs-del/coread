        ...
        <div className={`xiaowo-study ${readerTheme === 'eink' ? 'eink-theme' : `reader-theme-${readerTheme}`}`} style={{
            ...
            <div ref={contentRef} style={{
                ...
                onTouchMove={mode === 'reading' ? (e) => {
                    const start = touchStart.current;
                    if (!start || start.startedOnText || start.selectionClaimed) return;
                    ...
                    const maxDrag = width * 0.96;
                    const signed = start.direction === 'forward' ? Math.min(0, dx) : Math.max(0, dx);
                    const clamped = Math.max(-maxDrag, Math.min(maxDrag, signed));
                ...
                onTouchEnd={mode === 'reading' ? (e) => {
                    ...
                    if (start.selectionClaimed || window.getSelection()?.toString().trim()) return;
                    if (start.startedOnText) return;
                    if (start.startedAtEdge && !start.dragging) return;
                    if (!start.dragging) {
                        if (dt <= 500 && Math.abs(dx) >= 60 && Math.abs(dx) > Math.abs(dy)) {
                            reserveSelectionGesture(500);
                            if (dx < 0) goPage(1);
                            else goPage(-1);
                        }
                        return;
                    ...
                    const shouldComplete = progress >= 0.34 && Math.abs(dx) > Math.abs(dy);
                    const direction = start.direction === 'forward' ? 'forward' : 'backward';
                    const canTurn = direction === 'forward' ? page < totalPages - 1 : page > 0;
                    const targetX = shouldComplete && canTurn
                        ? (direction === 'forward' ? -width : width)
                        : 0;
                    pageEl.style.transition = `transform ${Math.max(180, Math.min(readerPageTurnDuration, 520))}ms cubic-bezier(0.22, 0.72, 0.2, 1)`;
                    ...
                    if (shouldComplete && canTurn) {
                        ...
                        }, Math.max(180, Math.min(readerPageTurnDuration, 520)));
                    } else {
                        window.setTimeout(() => {
                            pageEl.style.transition = '';
                            pageEl.style.transform = '';
                    ...
                    <>
                        {readingLoading ? (
                            ...
                            <div
                                ...
                                className={`coread-reader-page-transition ${readerPageTurnEffect === 'slide' ? `is-slide-${pageTurnDirection}` : readerPageTurnEffect === 'fade' ? 'is-fade' : ''}`}
